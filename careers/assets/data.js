...
(function () {
  ...
  const api = {
    ...
    // ----- 2FA (uses the same authenticator factor the HR app enrolled) -----
    async mfaStatus() {
      if (DEMO) return { needed: false };
      const s = await client();
      const { data, error } = await s.auth.mfa.getAuthenticatorAssuranceLevel();
      if (error) throw error;
      if (data.currentLevel === "aal2") return { needed: false };
      const { data: f } = await s.auth.mfa.listFactors();
      const totp = (f && f.totp || []).find(x => x.status === "verified");
      return { needed: true, factorId: totp ? totp.id : null };
    },
    async mfaVerify(factorId, code) {
      if (DEMO) return;
      const s = await client();
      const { data: ch, error: e1 } = await s.auth.mfa.challenge({ factorId });
      if (e1) throw e1;
      const { error: e2 } = await s.auth.mfa.verify({ factorId, challengeId: ch.id, code });
      if (e2) throw e2;
    },
    async session() {
